...
Confirmed delivered: broadcast 25159901 reached 2/2 recipients (1 open). Gmail Show-original reports SPF PASS (149.72.60.6), DKIM PASS on `avinashsangle.com`, DMARC PASS — email authentication is complete and externally verified. Earlier `recipients: 0` was Kit's `/stats` lagging the send, not a failure (bug 12 closed as not-a-bug).
Still landed in spam despite the three passes: zero domain sending reputation (first-ever send) plus thin, link-heavy content. Reworked `build_content()` — single CTA instead of three links to the same URL, and an optional `src/app/blog/<slug>/social/newsletter.md` supplies real paragraphs (falls back to the post description). Both paths dry-run verified. Logged as bug 13.
Deployed (`2fec665` pushed to main). Verified against the live site: signup renders on post pages (spot-checked 5), exactly once on `/blog`, `/contact` unchanged. Both recipients marked the first broadcast as not-spam. Remaining: rotate the Kit credentials, and hold DMARC at `p=none` until ~late Aug 2026 before considering `p=quarantine`.
Wired `newsletter.md` generation into `/promote-blogpost` as draft 2.6 (was previously only a reminder note). Format deliberately differs from the other drafts: plain prose, no header block, no `---BODY---` marker, no markdown — the script escapes HTML and does not render markdown, so any syntax would appear literally in the email. Updated the frontmatter description, the read-first script list, the "6 files" count, the commit-message template, and the quality checklist to match.

## 2026-07-26 — Trending Scout: retire Worker, serve as pushed static file (one URL)
Killed the duplicate: dashboard was live at both /projects/trending-scout (Vercel proxy) and trending-repo-scout.aavi-sangle.workers.dev, identical HTML, no canonical.
New: scout CI pushes its built dashboard into this repo at `src/app/projects/trending-scout/dashboard.html`; a `force-static` Route Handler (`route.ts`) reads it at build and serves it raw (full standalone doc — must bypass layout), graceful fallback if missing. File lives under src/ (not public/) so there's exactly one URL. Seeded an initial dashboard.html so it works on first deploy. Removed the external rewrite from next.config.ts; bumped sitemap.
Chose push over the ISR-pull variant (built first) because the scout repo is PRIVATE: pull needed a read token in Vercel + runtime fetch; push is a plain static file, no Vercel token, updates event-driven on scout push. Decision: docs/decisions/D-SEO-03-trending-scout-static-push. Verified: build ✓, route static, serves the real 97KB dashboard (correct title+canonical, not fallback).
Scout side (separate repo): build_dashboard.py emits canonical→bio + nav (absolute URLs) + SEO title/desc; new "Publish dashboard to bio site" step in hybrid-fetch.yml (guarded by BIO_REPO_TOKEN); Cloudflare automation deleted. Manual: create BIO_REPO_TOKEN (Contents:write on bio repo) in scout secrets; delete the CF Worker.
